  ...
  return (
    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" aria-hidden="true" className="shrink-0">
      <path
        ...
        strokeWidth="1.5"
      />
      <circle cx="12" cy="9.5" r="2.3" stroke="var(--teal-deep)" strokeWidth="1.5" />
    </svg>
  );
}
...
function MailIcon() {
  return (
    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" aria-hidden="true" className="shrink-0">
      <rect x="2.6" y="4.6" width="18.8" height="14.8" rx="1.5" stroke="var(--teal-deep)" strokeWidth="1.5" />
      <path d="M3.5 5.5L12 13L20.5 5.5" stroke="var(--teal-deep)" strokeWidth="1.5" strokeLinecap="round" />
    </svg>
  );
}
...
export function Footer() {
  return (
    <footer className="relative bg-paper text-body">
      ...
      {/* CTA strip — the one place in the footer with real weight, so the
          footer does more than list links. */}
      ...
      <div className="mx-auto max-w-[1280px] px-4 py-16 sm:px-6 lg:px-8">
        <div className="grid gap-y-12 sm:grid-cols-2 sm:gap-x-10 lg:grid-cols-[1.3fr_1fr_1fr_1fr] lg:gap-x-0">
          <div className="flex flex-col items-center text-center">
            ...
            />
            <p className="mt-4 font-display text-base italic text-muted">
              Key West real estate, done quietly.
            </p>
            <div className="mt-5 flex flex-col items-center gap-2.5">
              <div className="flex items-start gap-2.5">
                <PinIcon />
                <address className="font-sans text-sm not-italic text-body">
                  {brand.address.line1}
                  <br />
              ...
              <a
                href={brand.phone.href}
                className="flex items-center gap-2.5 font-sans text-sm text-body underline-offset-4 hover:text-teal-deep hover:underline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-teal-deep"
              >
                <PhoneIcon />
                {brand.phone.display}
              </a>
            ...
            <ul className="mt-4 flex flex-col gap-3">
              {primaryNav.map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className="font-sans text-sm text-body underline-offset-4 hover:text-teal-deep hover:underline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-teal-deep"
                  >
                    {item.label}
                  </Link>
                </li>
          ...
          <div className="lg:border-l lg:border-line lg:pl-10">
            ...
            <p className="mt-4 font-sans text-sm text-body">{brand.broker.title}</p>
