...
// A showcase card, not a transactional one — there's nothing to offer on or
// schedule a showing for once a home has sold, so this deliberately doesn't
// reuse ScrapedListingCard's "Make an Offer" CTA.
export function SoldListingCard({ listing }: { listing: ScrapedListing }) {
  return (
    <Link
      href={`${buildOwnListingUrl(listing)}?from=%2Fsold&fromLabel=Back+to+Recent+Sales`}
    ...
    >
      <div className="relative aspect-[3/2] overflow-hidden bg-paper">
        ...
        <span className="absolute left-4 top-4 rounded-full bg-ink/85 px-3 py-1 font-sans text-[11px] font-medium uppercase tracking-wide text-white shadow-sm backdrop-blur">
          Sold
        </span>
      </div>

      <div className="p-5">
        <p className="font-sans text-xs font-medium uppercase tracking-wide text-muted">Sold For</p>
        <p className="mt-1 font-display text-lg text-ink transition-colors group-hover:text-teal-deep">
